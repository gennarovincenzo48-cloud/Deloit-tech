              ...
              {PRODUCTS.map((p) => (
                ...
                >
                  <div className="h-40 bg-gray-50 rounded-md flex items-center justify-center overflow-hidden">
                    <img src={p.img || "/placeholder.svg"} alt={p.title} className="object-contain h-full w-full" />
                  </div>
                  <h3 className="mt-3 font-medium line-clamp-2">{p.title}</h3>
                  <p className="text-sm text-gray-500">{p.category}</p>
                  <div className="mt-3 flex items-center justify-between">
                    <div className="text-lg font-bold text-orange-600">₦{p.price.toLocaleString()}</div>
                    <button
                      onClick={() => addToCart(p)}
          ...
          {cart.length === 0 ? (
            ...
            <div className="mt-3 space-y-3">
              ...
              <div className="border-t pt-3 flex items-center justify-between font-bold">
                <div>Subtotal</div>
                <div className="text-orange-600">₦{subtotal.toLocaleString()}</div>
              </div>
              <div className="mt-3 space-y-2">
                ...
                <p className="text-xs text-gray-500 text-center">We'll send a confirmation to your email.</p>
              </div>
            </div>
          )}
          {message && <div className="mt-3 text-sm text-center text-gray-700 bg-orange-50 p-2 rounded">{message}</div>}
          <div className="mt-4 pt-4 border-t border-orange-50 text-xs text-gray-500">
            <strong className="block mb-2">Need help?</strong>
            <a
              href={`https://wa.me/09060571455?text=${encodeURIComponent("Hi Deloit Tech, I want to order")}`}
              className="text-orange-600 hover:text-orange-700 font-medium block mb-2"
            >
              WhatsApp: +234 906 057 1455
            </a>
            <a href="mailto:Ezenemebath@gmail.com" className="text-orange-600 hover:text-orange-700 font-medium">
      ...
      <div className="max-w-6xl mx-auto p-6 mt-10">
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-orange-50 text-center">
          ...
          <p className="text-sm text-gray-600 mb-4">
            DELOIT-TECH LIMITED is registered with the Corporate Affairs Commission of Nigeria. Certificate ID:
            CAC/7889960
          </p>
          <img
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            ...
            <div className="mt-2 text-xs">
              Registered & Verified — ID: <span className="font-mono">CAC/7889960</span>
            </div>
          </div>
    ...
    <div className="bg-white rounded-2xl p-8 shadow-sm border border-orange-50">
      <div className="flex flex-col md:flex-row items-center gap-8">
        <div className="flex-1">
          <h2 className="text-3xl font-bold text-balance">Premium Phone Accessories Made Easy</h2>
          <p className="mt-3 text-gray-600 leading-relaxed">
            Quality cases, chargers, cables and more. Government-verified retailer — buy with confidence and get fast
            nationwide delivery.
          </p>
        ...
        <div className="w-56 h-40 bg-orange-50 rounded-lg flex items-center justify-center">
          <svg width="140" height="100" viewBox="0 0 140 100" fill="none" xmlns="http://www.w3.org/2000/svg">
            <rect x="6" y="6" width="60" height="60" rx="8" stroke="#FB923C" strokeWidth="2" />
            <rect x="74" y="20" width="60" height="60" rx="8" stroke="#FB923C" strokeWidth="2" />
          </svg>
        </div>
      </div>
    </div>
